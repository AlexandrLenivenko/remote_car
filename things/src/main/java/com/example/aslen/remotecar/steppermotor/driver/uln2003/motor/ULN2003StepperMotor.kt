...
import com.example.aslen.remotecar.steppermotor.motor.StepperMotor
...
class ULN2003StepperMotor(in1GpioId: String,
    ...
    override fun getDegreesFromSteps(steps: Int, resolutionId: Int) = steps.toDouble() * CIRCLE_DEGREES.toDouble() / getStepsPerRevolution(resolutionId).toDouble()

    override fun getStepDurationMillisForRPM(rpm: Double, resolutionId: Int) = MINUTE_MILLIS.toDouble() / getStepsPerRevolution(resolutionId).toDouble() / rpm

    override fun getStepsPerRevolution(resolutionId: Int) = if (getResolution(resolutionId) == ULN2003Resolution.FULL) HALF_STEPS_PER_REVOLUTION else FULL_STEPS_PER_REVOLUTION

    override fun getExecutionDurationNanos(rpm: Double, resolutionId: Int, steps: Int) = (getStepDurationMillisForRPM(rpm, resolutionId) * steps.toDouble() * NANOS_IN_SECOND.toDouble()).toLong()

    override fun getMotorRunner(stepsToPerform: Int, direction: Direction, resolutionId: Int, executionDurationNanos: Long) = ULN2003MotorRunner(uln2003, stepsToPerform, direction, getResolution(resolutionId), executionDurationNanos)
